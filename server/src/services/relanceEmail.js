...
// ── Pièces jointes PDF référencées dans les règles générales ──────────────
//
// L'utilisateur insère une référence inline {{pdf:<token>|<nom>}} dans le
// texte des règles via le bouton "+" de l'éditeur. Au moment de la régénération,
// chaque référence est :
//   1. Remplacée inline par "<nom>" (avec guillemets) pour rester lisible.
//   2. Le texte extrait du PDF est appendé une fois par token unique à la fin
//      du system prompt, sous l'en-tête `=== <nom> ===`.
//
...
const publicFileTextCache = new Map()

function loadPublicFileText(token) {
  if (!token) return null
  const row = db.prepare(
    'SELECT stored_name, mime_type, original_name FROM public_files WHERE token = ?'
  ).get(token)
  if (!row) return null
  ...
  const cached = publicFileTextCache.get(token)
  if (cached && cached.storedName === row.stored_name && cached.mtimeMs === mtimeMs) {
    return { text: cached.text, originalName: row.original_name }
  }
  ...
  try {
    ...
    console.error(`[relanceEmail] Échec extraction PDF (token=${token}) :`, e.message)
    return null
  }

  text = (text || '').trim()
  if (!text) return null
  publicFileTextCache.set(token, { storedName: row.stored_name, mtimeMs, text })
  return { text, originalName: row.original_name }
}

// Parse les références {{pdf:token|nom}} d'un bloc de règles et retourne :
//   - resolvedText : règles avec chaque référence remplacée par `"<nom>"`
//   - attachments : array dédupliqué par token, [{ name, text }] dans l'ordre
//                   d'apparition. `name` = label saisi dans la chip, fallback
//                   sur original_name du PDF.
function resolvePdfReferences(rulesText) {
  const text = rulesText || ''
  const attachments = []
  const seen = new Map()
  const resolvedText = text.replace(PDF_REF_RE, (_full, token, label) => {
    const file = loadPublicFileText(token)
    if (!file) {
      // Token cassé ou PDF supprimé : on remplace par le label saisi (ou un
      // marker générique) pour ne pas laisser de syntaxe brute dans le prompt.
      return `"${(label || 'document').trim()}"`
    }
    const displayName = (label || '').trim() || file.originalName || 'document'
    if (!seen.has(token)) {
      seen.set(token, displayName)
      attachments.push({ name: displayName, text: file.text })
    }
    return `"${displayName}"`
  })
...
export async function regenerateEmail({ qcId, temperature = 0.7, generalRules, specificInstructions }) {
  ...
  // System prompt = règles générales utilisateur (avec références {{pdf:...}}
  // résolues en `"nom"` inline) + un bloc par PDF unique référencé, sous
  // l'en-tête `=== nom ===`. Plus de brand brief hardcodé : tout vit dans les
  // règles générales éditables côté UI.
  const general = (generalRules || '').trim()
  const { resolvedText, attachments } = resolvePdfReferences(general)
  const parts = []
  if (resolvedText) parts.push(`RÈGLES GÉNÉRALES (définies par l'utilisateur, à respecter) :\n${resolvedText}`)
  for (const att of attachments) {
    parts.push(`=== ${att.name} ===\n${att.text}`)
  }
  const systemMessage = parts.join('\n\n') || 'Génère un courriel de relance professionnel.'
