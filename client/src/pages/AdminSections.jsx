...
// La charge Linux (load average) se lit en nombre de processus en attente de CPU :
// saturation à partir de `cores`. On l'affiche en % de la capacité de la machine.
function CpuStat({ label, load, cores }) {
  const pct = cores ? Math.round(load / cores * 100) : null
  const color = pct == null ? 'text-slate-800' : pct >= 100 ? 'text-red-600' : pct >= 75 ? 'text-amber-600' : 'text-slate-800'
  return (
    <div className="bg-slate-50 rounded-xl p-3 text-center" title={`Charge ${load.toFixed(2)} sur ${cores} cœur${cores > 1 ? 's' : ''} — saturation à ${cores}.00`}>
      <div className={`text-lg font-bold ${color}`}>{pct == null ? load.toFixed(2) : `${pct}%`}</div>
    ...
    <div className="space-y-6">
      ...
      <div className="card p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
          <CpuStat label="CPU 1 min" load={data.cpu.load1} cores={data.cpu.cores} />
          <CpuStat label="CPU 5 min" load={data.cpu.load5} cores={data.cpu.cores} />
          <CpuStat label="CPU 15 min" load={data.cpu.load15} cores={data.cpu.cores} />
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input type="password" value={pw} onChange={e => setPw(e.target.value)} className="input" required />
      </div>
      <div>
        <label className="label">Confirmer</label>
        <input type="password" value={pw2} onChange={e => setPw2(e.target.value)} className="input" required />
      </div>
      {error && <p className="text-red-600 text-sm">{error}</p>}
      <div className="flex justify-end gap-3 pt-2">
        <button type="button" onClick={onClose} className="btn-secondary">Annuler</button>
...
export function UtilisateursContent({ currentUser }) {
    ...
    <>
      ...
      <div className="mb-6">
        {ready ? (
          ...
          <div className="card p-8 text-center text-sm text-slate-400"><Spinner size="xs" label="Chargement…" /></div>
        )}
      </div>
