...
router.get('/:id', (req, res) => {
  const product = db.prepare('SELECT * FROM products WHERE id = ? AND tenant_id = ?').get(req.params.id, req.user.tenant_id);
...
router.post('/', (req, res) => {
  ...
  const id = uuidv4();
  db.prepare(
    `INSERT INTO products (id, tenant_id, sku, name_fr, name_en, type, unit_cost, price_cad, stock_qty, min_stock, order_qty, supplier, procurement_type, weight_lbs, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(id, req.user.tenant_id, sku || null, name_fr, name_en || null, type || null,
  ...
  const run = db.transaction(() => {
    db.prepare(
      `INSERT INTO stock_movements (id, tenant_id, product_id, type, qty, reason, reference_id, user_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(movId, req.user.tenant_id, req.params.id, type, parseInt(qty), reason || null, reference_id || null, req.user.id);
