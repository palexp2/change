...
router.post('/', (req, res) => {
  ...
  const id = uuidv4();
  db.prepare(
    `INSERT INTO projects (id, name, company_id, contact_id, assigned_to, type, status, probability, value_cad, monthly_cad, nb_greenhouses, close_date, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(id, name, company_id || null, contact_id || null, assigned_to || null,
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(
    ...
    close_date || null, refusal_reason || null, notes || null, req.params.id);
...
router.patch('/:id/status', (req, res) => {
  ...
  db.prepare(`UPDATE projects SET status=?, refusal_reason=?, updated_at=datetime('now') WHERE id = ?`)
