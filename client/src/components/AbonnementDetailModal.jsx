                  ...
                  {details.invoices.map((inv, i) => (
                    <div key={i} className={`px-4 py-2.5 ${inv.facture_id ? 'hover:bg-slate-50 cursor-pointer' : ''}`} onClick={() => { if (inv.facture_id) { onClose?.(); window.location.href = `/erp/factures/${inv.facture_id}` } }}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <span className={`text-xs font-mono ${inv.facture_id ? 'text-indigo-600 hover:underline' : 'text-slate-700'}`}>{inv.number || '—'}</span>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {inv.pdf && <a onClick={e => e.stopPropagation()} href={inv.pdf} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline text-xs inline-flex items-center gap-1"><FileText size={11} /></a>}
                        </div>
                      </div>
