// Vérifie le side-peek (RecordPeekDrawer) sur la table Contacts :
//  - un clic sur une ligne ouvre le drawer latéral SANS quitter /contacts
//  - le drawer rend la fiche ContactDetail embarquée (labels du formulaire)
...
describe('Contacts — side-peek drawer', () => {
  ...
  async function openFirstRowPeek() {
    ...
    // Clic sur la cellule "nom" (1re colonne, sans lien) pour éviter le lien entreprise.
  ...
  test('clic sur une ligne ouvre le drawer sans naviguer', async () => {
    ...
    assert.ok(path.includes('/contacts'), 'reste sur /contacts')
    assert.ok(!/\/contacts\/[^/]+$/.test(path),
      `ne doit pas avoir navigué vers la fiche (url=${page.url()})`)
    // Le corps rend la fiche embarquée : labels du formulaire contact.
    const body = await page.locator('[data-testid="record-peek-body"]').innerText()
    assert.ok(/Courriel/i.test(body), 'le drawer affiche le champ Courriel')
    assert.ok(/Entreprises/i.test(body), 'le drawer affiche la section Entreprises')
  ...
  test('bouton « ouvrir en grand » navigue vers la fiche complète', async () => {
    ...
    await page.waitForURL(u => u.toString().includes(`/contacts/${rowId}`), { timeout: 8000 })
    assert.ok(page.url().includes(`/contacts/${rowId}`), 'navigué vers /contacts/:id')
