...
// Un nom de champ Airtable très long ne doit pas briser la mise en page de la
// modale « Mapping des champs » (AirtableCoreMapModal) : le nom est tronqué
...
describe('Paies — nom de champ Airtable long dans la modale de mapping', () => {
  ...
  test('le nom long est tronqué + tooltip, la modale ne déborde pas', async () => {
    await page.goto(URL + '/paies', { waitUntil: 'domcontentloaded' })
    // Bouton « Mapping Airtable » dans l'en-tête (pattern standard des pages)
    ...
    // 3. La mise en page tient : aucun débordement horizontal dans la modale,
    //    et chaque rangée reste dans la largeur du dialogue.
    const overflow = await dialog.evaluate(el => el.scrollWidth - el.clientWidth)
    assert.ok(overflow <= 1, `la modale ne doit pas défiler horizontalement (débordement: ${overflow}px)`)
    const dialogBox = await dialog.boundingBox()
    const triggerBox = await trigger.boundingBox()
    assert.ok(
      triggerBox.x + triggerBox.width <= dialogBox.x + dialogBox.width + 1,
      'le picker au nom long doit rester dans la largeur de la modale'
    )
