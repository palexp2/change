...
### Règle — nettoyer les records créés par les tests E2E

Les tests tournent contre la **vraie DB** (`server/data/erp.db`) — il n'y a pas de base de test isolée. **Tout record créé pendant un test doit être supprimé à la fin de ce test**, sinon la DB se pollue commit après commit (users `e2e-…@example.com`, commandes orphelines, factures fantômes, etc.).

Pattern obligatoire :
- Utiliser le hook `after()` de `node:test` pour faire le cleanup, **même si le test a échoué** (le hook s'exécute quand même).
- Préférer `DELETE` via l'API admin (route protégée, side effects propres) plutôt qu'un `UPDATE … deleted_at` direct.
- Donner aux records un préfixe identifiable (`e2e-…`, `E2E …`, ou suffixé par `Date.now()`) pour les retrouver facilement en cas de cleanup raté.
- Si un test échoue avant le cleanup, le record résiduel doit pouvoir être supprimé manuellement à la main via l'API ou la page admin — ne **jamais** committer un script de "rattrapage" qui purge la DB en masse.

Exemple minimal :
```js
let createdId
before(async () => {
  const r = await api(adminToken, 'POST', '/foo', { name: `E2E ${Date.now()}` })
  createdId = r.body.id
})
after(async () => {
  if (createdId) await api(adminToken, 'DELETE', `/foo/${createdId}`)
})
```

### Règle — sauvegarder/restaurer les configurations utilisateur écrasées par les tests E2E

Quand un test E2E modifie une **configuration ou préférence existante** (et non un record qu'il vient de créer) — règles IA, vues de DataTable, préférences utilisateur, paramètres globaux, valeurs Admin, instructions de prompt, etc. — il doit **lire la valeur courante avant** de la modifier et **la restaurer** dans le hook `after()`. S'applique à *toute* zone de l'app : pages de paramètres, sidebars de config, modales d'édition, formulaires Admin. Pas seulement aux Règles générales de Relance Qualification.

Pourquoi : la DB de tests = la DB de prod. Sans restauration, chaque run écrase les configurations du user (« Test 1779…: signer P.A.P. ») et il faut les retaper à la main après chaque commit. La règle « créer un préfixe identifiable » ne s'applique pas ici parce qu'on n'a pas créé un record nouveau — on a écrasé un champ existant.

Pattern obligatoire :
```js
let originalValue
before(async () => {
  const r = await api(token, 'GET', '/some/settings')
  originalValue = r.body.theField
})
after(async () => {
  // Restaure toujours, même si le test a échoué — sinon la valeur de test reste en DB.
  if (originalValue !== undefined) {
    await api(token, 'PUT', '/some/settings', { theField: originalValue })
  }
})
```

Si la valeur originale n'est pas accessible via API (cas rare), passer par une lecture directe en SQLite readonly au lieu d'écraser sans backup. Ne jamais écraser une configuration sans avoir d'abord capturé l'état initial.
