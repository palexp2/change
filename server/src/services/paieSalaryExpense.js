...
//                     − téléphone Martin (taxes incluses).
//
...
export function computePaieSalaryExpense(paieId, input = {}) {
  ...
  // MÊME dépense : on la sort des remboursements pour ne la compter qu'une
  // fois, sur le compte téléphone (76000, code TPS/TVQ). Pas de ligne
  // téléphone si la paie ne contient pas ce remboursement.
  ...
  const base = round2(bank - reimbTotal - phone)
  ...
  return {
    ...
    total: round2(lines.reduce((s, l) => s + l.amount, 0)), // = bank_amount (taxes incluses)
    lines, warnings,
    memo: `Paie – ${period}`,
...
async function resolveRefs(preview) {
  ...
  // explicitement un montant taxes incluses (QB ignore TaxInclusive sur les
  // lignes de Purchase et recalcule la taxe PAR-DESSUS le montant).
  ...
  const purchaseRatesFor = (name) => {
    const code = findTaxCode(name)
    return (code.PurchaseTaxRateList?.TaxRateDetail || [])
...
export async function pushPaieSalaryExpense(paieId, input = {}) {
  ...
  try {
    ...
    // QB ignore GlobalTaxCalculation:TaxInclusive sur les Purchase et recalcule
    // la taxe PAR-DESSUS le montant de ligne (25 $ tx incl. devenait 28,74 $).
    // On ventile donc nous-mêmes : montant HT sur la ligne + TxnTaxDetail
    // explicite (gotcha : Purchase QB exige des TaxLine pour un montant exact).
    ...
    const qbLines = preview.lines.map(l => {
      ...
      if (rates.length) {
        // l.amount est taxes incluses → extraire le HT et ventiler la taxe.
        const totalPct = rates.reduce((s, r) => s + r.pct, 0)
        let ht = round2(l.amount / (1 + totalPct / 100))
        const taxes = rates.map(r => ({ ...r, tax: round2(ht * r.pct / 100) }))
        ht = round2(l.amount - taxes.reduce((s, t) => s + t.tax, 0)) // dérive d'arrondi → HT
        for (const t of taxes) {
          const cur = taxTotals.get(t.rateRef) || { pct: t.pct, net: 0, tax: 0 }
          cur.net = round2(cur.net + ht); cur.tax = round2(cur.tax + t.tax)
    ...
    const purchase = {
      ...
      Line: qbLines,
      ...(taxLines.length ? {
        TxnTaxDetail: { TotalTax: round2(taxLines.reduce((s, t) => s + t.Amount, 0)), TaxLine: taxLines },
      } : {}),
    }
