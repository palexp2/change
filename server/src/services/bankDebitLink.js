...
export async function confirmDebtPaymentsFromBank(debtId = null) {
  ...
  for (const debt of debts) {
    ...
    for (const p of payments) {
      const expected = round2((Number(p.principal) || 0) + (Number(p.interest) || 0))
      ...
      const hit = found.match && !found.match.pending
        ...
        ? found.match : null
      if (!hit) continue
      db.prepare("UPDATE lt_debt_payments SET bank_txn_id=?, updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=?")
        .run(hit.id, p.id)
      used.push(hit.id)
      linked.push({ debt: debt.label, payment_id: p.id, payment_date: p.payment_date, txn_id: hit.id, amount: hit.amount })
    }
  }
  return linked
}
